...
document.addEventListener('DOMContentLoaded', function () {
  ...
  //       e.stopPropagation(); // boshqa eventlar bloklansin
  ...
  // // Tashqi joy bosilganda dropdownni yopish
  // document.addEventListener("click", (e) => {
  //   // Agar dropdown ochiq bo'lsa va bosilgan joy dropdown ichida bo‘lmasa
  //   if (
  //     dropdownMenu.classList.contains("show") &&
  //     !dropdownMenu.contains(e.target) &&
  //     ![...openDropdownMenu].some(btn => btn.contains(e.target))
  //   ) {
  ...
  openDropdownMenu.forEach(btn => {
    // sichqoncha tugma ustiga kirganda menyuni ko'rsat
    ...
    // sichqoncha tugma ustidan chiqqanda menyuni yopish (lekin faqat agar menyudan ham chiqqan bo‘lsa)
    btn.addEventListener("mouseleave", (e) => {
      setTimeout(() => {
        if (!dropdownMenu.matches(':hover') && !btn.matches(':hover')) {
          dropdownMenu.classList.remove("show");
          btn.classList.remove("active");
          // overlay.classList.remove('show');
        }
      }, 200); // ozgina kechikish kerak, foydalanuvchi menyuga o‘ta olish uchun
    });
  });
